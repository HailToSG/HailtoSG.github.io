...
function initializeMap() {
    // Map options
    var opts = {
        center: new google.maps.LatLng(44.594960,33.475786),
        zoom: 16,
		zoomControl: false,
    ...
    google.maps.event.addListener(map, 'click', function(event) {
        ...
        if (nodes.length >= 9) {
            alert('Можно добавить не более 9');
            return;
        }
    ...
    function getMyLocation(myLocationDiv, map) {
        ...
        myLocationBtn.innerHTML = 'Мое местоположение';
        ...
        for (originNodeIndex in distanceData.rows) {
            ...
            for (destinationNodeIndex in nodeDistanceData) {
                if (durations[originNodeIndex][destinationNodeIndex] = nodeDistanceData[destinationNodeIndex].duration == undefined) {
                    alert('Ошибка: не могу проложить маршрут');
                    return;
                }
...
function clearMap() {
    ...
    $('#destinations-count').html('&nbsp;');
	$('#generations-passed').html('&nbsp;');
	$('#best-time').html('&nbsp;');
}
    ...
    $('#find-route').click(function() {    
        if (nodes.length < 2) {
            if (prevNodes.length >= 2) {
                ...
                alert('Кликните по карте, чтобы установить точку маршрута');
                return;
            }
        }
        ...
        getDurations(function(){
            ...
            ga.evolvePopulation(pop, function(update) {
                ...
                $('#best-time').html((update.population.getFittest().getDistance() / 60).toFixed(2) + ' Мин.');
